...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var style = {
			...
			sectHead: { display: "flex", gap: 8, alignItems: "center", cursor: "pointer", userSelect: "none", background: "none", border: "none", padding: 0, font: "inherit", textAlign: "left", width: "100%" },
			chevron: { color: "var(--dsw-alias-label-tertiary)", fontSize: 12, width: 14 },
		...
		// Collapsible card section. Static placement only (hooks rule).
		function Section(props) {
			var pair = useStatePair(props.open !== false);
			...
			return h(
				...
				h(
					"button",
					{ style: style.sectHead, onClick: function () { setOpen(!open); }, "aria-expanded": open },
					h("span", { style: style.chevron }, open ? "▾" : "▸"),
					h("h3", { style: style.cardTitle }, props.title),
				),
				open ? props.children : null,
			);
		}
		// One-line labeled row: label + control + hint.
		function FieldRow(props) {
			return h(
				"div",
				{ style: Object.assign({}, style.row, { justifyContent: "space-between" }) },
				h(
					"div",
					{ style: style.row },
			...
			return h(
				...
				h(
					...
					h(
						...
						h("button", { style: style.secondaryButton, onClick: function () { load("/mnt/raid0/GGUF"); } }, "Models"),
		...
		function LocalModelsSection() {
			...
			useEffect(function () {
				...
				return undefined;
			}, [selectedPath, ctx, mtp, mmprojPath, moeMode, nCpuMoeText]);
			...
			var modelBody = h(
				...
				h(
					...
					selectedPath
						? h("span", { style: { fontFamily: "var(--ds-font-family-code)", fontSize: 12, wordBreak: "break-all" } }, selectedPath)
				...
				selectedPath ? h(
					...
					meta ? h(
						"div",
						{ style: { display: "flex", flexDirection: "column", gap: 2 } },
						h("span", { style: { fontSize: 13, fontWeight: 500 } }, pickedName + (meta.name ? "  ·  " + meta.name : "")),
						h("span", { style: style.mono }, (meta.fileTypeName || "?") + " · " + (meta.nLayers != null ? meta.nLayers + " layers" : "?") + " · " + (meta.nEmbd != null ? "embd " + fmtInt(meta.nEmbd) : "") + " · head_dim " + (meta.headDim || "?") + " · train_ctx " + (trainCtx ? fmtCtx(trainCtx) : "?")),
						h("span", { style: style.small }, "file " + fmtGiB(metaRoot.fileSize) + " · weights " + fmtGiB(metaRoot.weightsBytes)),
						meta.isMoe ? h("span", { style: style.mono }, "MoE · " + String(meta.expertCount != null ? meta.expertCount : "?") + " experts · top-" + String(meta.expertUsedCount != null ? meta.expertUsedCount : "?") + " active" + (meta.expertSharedCount > 0 ? " (+" + meta.expertSharedCount + " shared)" : "")) : null,
					) : null,
				...
				meta ? h(
					"div",
					{ style: style.row },
					h("span", { style: style.cardTitle }, "Context (steps of " + fmtCtx(CTX_STEP) + ", max " + fmtCtx(ctxMax) + ")"),
				...
				meta ? h(FieldRow, {
					...
					hint: "fixed MTP draft, 3 max — deeper collapses at large ctx · KV is always Q8_0 (K) / Q4_0 (V)",
				}) : null,
				meta ? h(FieldRow, {
					...
					hint: "server default — dsh can also request effort per message after registering",
				}) : null,
				h(
					"div",
					{ style: style.row },
			...
			// ---- render: VRAM estimate ----
			var estimateBody;
			if (!estimate) {
				estimateBody = null;
			} else if (estimate.unavailable) {
				estimateBody = h("p", { style: style.small }, "Attention dimensions missing from the GGUF header — KV estimate unavailable.");
			} else {
						...
						profiles.map(function (p) {
							return h("div", { key: String(p.id || p.name), style: { border: "1px solid var(--dsw-alias-border-l3)", borderRadius: 10, padding: "8px 10px", display: "flex", flexDirection: "column", gap: 6 } },
								h("div", { style: { display: "flex", flexDirection: "column", gap: 2 } },
									...
									h("span", { style: style.small }, String(p.modelPath || "").split("/").pop() + " · ctx " + fmtCtx(p.ctx || 0) + " · " + (p.effort || "medium") + (p.mmprojPath ? " · vision" : "") + (p.cpuMoe === true ? " · experts CPU" : (p.nCpuMoe > 0 ? " · experts cpu×" + p.nCpuMoe : "")) + (p.expertUsed > 0 ? " · top-" + p.expertUsed : "")),
								),
								h("div", { style: style.row },
			...
			var routerBody = h(
				"div",
				{ style: { display: "flex", flexDirection: "column", gap: 8 } },
				h("p", { style: style.small }, "One endpoint — models from your saved profiles load on demand (llama.cpp router mode)."),
				status.mode === "router"
					? h("div", { style: { display: "flex", flexDirection: "column", gap: 8 } },
				...
				h("div", { style: style.row },
					...
					h("button", { style: style.secondaryButton, onClick: doRegister, disabled: !running || busy }, "Register in dsh"),
				),
			);
			...
			var actionsRow = h(
				...
				running ? h("button", { style: style.button, onClick: doRegister, disabled: busy }, "Register in dsh") : null,
			...
			return h(
				...
				h(Section, { title: "Profiles", open: true }, profilesBody),
				h(Section, { title: "Router", open: true }, routerBody),
				h(Section, { title: "Model", open: true }, modelBody),
				estimate ? h(Section, { title: "VRAM estimate", open: true }, estimateBody) : null,
				actionsRow,
				statusCard,
				...
				pickerOpen ? h(FileBrowserModal, { allFiles: false, onClose: function () { setPickerOpen(false); }, onPick: function (p) { setPickerOpen(false); setSelectedPath(p); setMetaRoot(null); fitAppliedRef.current = false; }, startDir: "" }) : null,
				mmPickerOpen ? h(FileBrowserModal, { allFiles: true, onClose: function () { setMmPickerOpen(false); }, onPick: function (p) { setMmPickerOpen(false); setMmprojPath(p); setMmMetaRoot(null); clearedMmprojRef.current = false; }, startDir: "" }) : null,
