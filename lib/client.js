...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function LocalModelsSection() {
			...
			var nocapPair = useStatePair(false);
			var nocap = nocapPair[0], setNocap = nocapPair[1];
			...
			var LS_NOCAP = "dsh-local-models:ignore-softcap";
			...
			useEffect(function () {
				try {
					...
					if (localStorage.getItem(LS_NOCAP) === "1") setNocap(true);
			...
			var refresh = useCallback(function () {
				get("/local-models/status").then(function (body) {
					...
					if (body.modelPath && !selectedPathRef.current) {
						...
						if (body.ignoreCtxCap != null) setNocap(body.ignoreCtxCap === true);
			...
			useEffect(function () {
				try {
					...
					localStorage.setItem(LS_NOCAP, nocap ? "1" : "0");
				...
				return undefined;
			}, [selectedPath, ctx, mtp, nocap, mmprojPath, moeMode, nCpuMoeText, tab]);
			...
			var doRun = function () {
				setBusy(true);
				setError(null);
				post("/local-models/run", Object.assign({ path: selectedPath, ctx: Math.floor(Number(ctxText)) || ctx, mtp: mtp, mmproj: mmprojPath || null, effort: effort, ignoreCtxCap: nocap, mmprojCpu: mmprojCpu }, moePayload()))
					.then(function (res) {
			...
			var doSaveProfile = function () {
				...
				setBusy(true);
				setError(null);
				post("/local-models/profiles", Object.assign({ name: name, modelPath: selectedPath, ctx: Math.floor(Number(ctxText)) || ctx, mtpHeads: mtp, mmprojPath: mmprojPath || null, effort: effort, ignoreCtxCap: nocap }, moePayload()))
					.then(function (res) {
			...
			var modelBody = h(
				...
				meta && mtp > 0 ? h(
					...
					h("label", { style: { display: "flex", flexDirection: "row", alignItems: "center", gap: 6 } },
						h("input", { type: "checkbox", checked: nocap, onChange: function (ev) { setNocap(ev.target.checked); } }),
						h("span", { style: style.small }, "ignore the MTP ctx softcap" + (ctx > VRAM_CTX_LIMIT ? " — draft is OFF above " + fmtCtx(VRAM_CTX_LIMIT) : "")),
					),
				) : null,
				h(
					"div",
						...
						profiles.map(function (p) {
							return h("div", { key: String(p.id || p.name), style: { border: "1px solid var(--dsw-alias-border-l3)", borderRadius: 10, padding: "8px 10px", display: "flex", flexDirection: "column", gap: 6 } },
								h("div", { style: { display: "flex", flexDirection: "column", gap: 2 } },
									...
									h("span", { style: style.small }, String(p.modelPath || "").split("/").pop() + " · ctx " + fmtCtx(p.ctx || 0) + " · " + (p.effort || "medium") + (p.mtpHeads > 0 ? " · mtp " + p.mtpHeads : "") + (p.ignoreCtxCap === true ? " · nocap" : "") + (p.mmprojPath ? " · vision" : "") + (p.cpuMoe === true ? " · experts CPU" : (p.nCpuMoe > 0 ? " · experts cpu×" + p.nCpuMoe : "")) + (p.expertUsed > 0 ? " · top-" + p.expertUsed : "")),
								),
								h("div", { style: style.row },
