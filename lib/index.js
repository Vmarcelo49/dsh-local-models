...
const FILE_TYPE_NAMES = {
	...
	15: "Q8_K", 16: "BF16", 23: "IQ4_NL", 27: "IQ4_XS", 28: "I8", 29: "I16", 30: "I32",
};
...
function createManager() {
	...
	async function run(modelPath, ctxSize, mtpHeads, mmprojPath, effort, mmprojCpu = MMPROJ_CPU, moe = null) {
		...
		const specArgs = mtp > 0 && ctx <= VRAM_CTX_LIMIT
		...
		return launch(args, {
			...
			logNote: "starting " + basename(modelPath) + " (ctx " + ctx + ", mtp " + mtp + ", effort " + effortLevel + (moeMeta?.isMoe ? ", experts " + (moeCfg.cpuMoe ? "cpu" : moeCfg.nCpuMoe > 0 ? "cpu-first-" + moeCfg.nCpuMoe : "gpu") + (moeCfg.expertUsed != null ? " top-" + moeCfg.expertUsed : "") : "") + ", mmproj " + (mmproj ? basename(mmproj) + (mmprojCpu ? " (cpu)" : " (gpu)") : "none") + ")",
		});
	}
...
export function normalizeProfileConfig(raw) {
	...
	return { modelPath, ctx, mtpHeads, mmprojPath, effort, ...moe };
}
...
export function generateRouterPresets(profiles) {
	...
	for (const p of profiles) {
		...
		// to the ctx ceiling, dropped above it.
		...
		if (mtp > 0 && ctxSize <= VRAM_CTX_LIMIT) {
	...
	const routeSpec = [
		...
		},
		{
			...
			const moe = {
				...
				const result = await manager.run(path, Number.isFinite(ctx) ? ctx : 8192, Number.isFinite(mtp) ? mtp : 0, mmproj, effort, body.mmprojCpu === false ? false : MMPROJ_CPU, moe);
